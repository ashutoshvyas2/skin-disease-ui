...
if os.path.exists(MODEL_PATH):
    ...
    try:
        # We must include the custom FocalLoss class here
        model = tf.keras.models.load_model(MODEL_PATH, custom_objects={'FocalLoss': FocalLoss})
        print("✅ Model loaded successfully!")
        # These will work now that model is NOT None
        print(f"--- MODEL INPUT SHAPE: {model.input_shape} ---")
    except Exception as e:
        print(f"❌ ERROR LOADING MODEL: {e}")
else:
    print(f"❌ CRITICAL ERROR: {MODEL_PATH} is missing from C:\\ui ux")
...
def predict():
    ...
    try:
        ...
        # Using built-in EfficientNet scaling logic
        # If your model used preprocess_input, we do it here:
